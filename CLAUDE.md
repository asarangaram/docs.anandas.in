...
- **Nav is derived from the directory tree by the `awesome-nav` plugin, not the `mkdocs.yml` `nav:` key** (there is no `nav:` in `mkdocs.yml`). Per-directory `.nav.yml` files set section titles and ordering — the top-level one is `docs/.nav.yml`. A new page appears on its own once its file exists; you only touch a `.nav.yml` to title or order a section. Gotcha: a `.nav.yml` that names a directory which no longer exists (e.g. after moving a section out) fails `--strict` with "doesn't match any files" — remove the stale entry. URLs derive from *file paths*, not nav position: reordering in a `.nav.yml` is free, moving a file on disk changes its URL.
